...
import { FileHashesRequest, FileHashesResponse, FileMetadataRequest, FileMetadataResponse, SearchFileRequest, SearchFilesResponse } from './types';
...
export class GetFilesApi extends BaseApi {
  ...
  async searchFiles(args: SearchFileRequest): Promise<SearchFilesResponse> {
    return await this.request({
      method: 'GET',
      endpoint: 'get_files/search_files',
      args: { ...args, tags: args.tags },
    });
  }

  async getFileHashes(args: FileHashesRequest): Promise<FileHashesResponse> {
    return await this.request({ method: 'GET', endpoint: 'get_files/file_hashes', args });
  }

  // todo
  async getFileMetadata(args: FileMetadataRequest): Promise<FileMetadataResponse> {
